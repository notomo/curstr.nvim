...
function! s:suite.before_each()
    ...
    call curstr#custom#source_option('togglable/word', 'words', [])
...
function! s:suite.toggle()
    ...
    call setpos('.', [0, 1, 1, 0])
    call curstr#custom#source_option('togglable/word', 'words', ['public', 'protected', 'private'])
...
function! s:suite.append()
    ...
    Curstr togglable/word -action=append
...
function! s:suite.normalized_toggle()
    call append(0, 'True')
    call setpos('.', [0, 1, 1, 0])
    call curstr#custom#source_option('togglable/word', 'words', ['true', 'false'])
    call curstr#custom#source_option('togglable/word', 'normalized', v:true)
...
function! s:suite.char_pattern_option()
    ...
    call setpos('.', [0, 1, 1, 0])
    call curstr#custom#source_option('togglable/word', 'words', ['foo:test', 'foo;test'])
    call curstr#custom#source_option('togglable/word', 'char_pattern', '[:alnum:]_;:')
...
function! s:suite.nomodifiable()
    ...
    call setpos('.', [0, 1, 1, 0])
    call curstr#custom#source_option('togglable/word', 'words', ['public', 'protected'])
