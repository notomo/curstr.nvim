...
class SourceExecuteInfo(object):
    def __init__(
        self, source_name: str, source_options: Options, execute_options: Options
    ) -> None:
...
class ExecuteInfo(object):
    ...
    def __init__(
        ...
        user_options = vim.call("curstr#custom#get_execute_options", "_")
        default_execute_options = {**self._DEFAULT_EXECUTE_OPTIONS, **user_options}

        source_names = sorted(set(source_names), key=source_names.index)

        self._source_execute_infos = list(
            chain.from_iterable(
                (
        ...
        self._action_options = {**user_action_options, **action_options}
    ...
    def from_arg_string(
        ...
        source_names = list(filter(lambda x: len(x) != 0, source_names))
    ...
    def _resolve_source_name(
        ...
        source_options = self._vim.call("curstr#custom#get_source_options", source_name)
        ...
        source_options = {**source_options, **preferred_source_options}
        ...
        if len(aliased_source_name) == 0:
            return [
                SourceExecuteInfo(source_name, source_options, source_execute_options)
            ]

        infos = (
            self._resolve_source_name(
                name, source_options, source_execute_options, default_execute_options
            )
