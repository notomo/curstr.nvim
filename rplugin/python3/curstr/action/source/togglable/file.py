...
class Source(Base):
    ...
    def get_options(self):
        return {"pattern_groups": [], "offset": 1, "create": False}

    def _dispatch(self, path: str):
        group = self._dispatcher.dispatch_one(FileDispatcher.File, path)
        if not group.is_nothing():
            return group

        if self.get_option("create"):
            return self._dispatcher.dispatch_one(FileDispatcher.NewFile, path)
    ...
    def _get_group(
        ...
        for i in range(1, limit + 1):
            ...
            match_path = pattern.replace(self.PLACE_HOLDER, "{}").format(*matches)
            if to_abspath:
                match_path = join(self._vim.call("expand", "%:p:h"), match_path)
