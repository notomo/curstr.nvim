...
from curstr.exception import ActionModuleNotFoundException, SourceNotFoundException
...
class Importer(Echoable, MetaPathFinder):
    ...
    def _get_spec(self, module_paths: List[str]):
        ...
        if os.path.isfile(file_path):
            ...
            # namespace package
            spec = spec_from_loader("curstr.action.{}".format(module_path), None)
    ...
    def get_sources(self, execute_info: ExecuteInfo):
        return (
            self._get_source(info.source_name, info.use_cache)(
                info, Cursor(self._vim, info)
            )
            for info in execute_info.source_execute_infos
        )

    def _get_source(self, source_name: str, use_cache: bool):
        if use_cache and source_name in self._sources:
            return self._get_source_instance(self._sources[source_name], use_cache)

        module_name = "curstr.action.source.{}".format(".".join(source_name.split("/")))
        module = self._import(module_name)
        if hasattr(module, "Source"):
            cls = module.Source
            self._sources[source_name] = cls
            return self._get_source_instance(cls, use_cache)

        raise SourceNotFoundException(source_name)

    def _get_source_instance(self, cls: Type[Source], use_cache: bool):
        dispatcher = self._load_dispatcher(cls.DISPATCHER_CLASS, use_cache)
    ...
    def _reload_action_group(self):
        ...
        groups = [
            (name, cls)
            for name, cls in group_module.__dict__.items()
            if (
                isinstance(cls, type)
                and cls.__module__.startswith("curstr.action.group")
            )
        ]
        for name, cls in groups:
            reloaded = self._import(cls.__module__)
            group_module.__dict__[name] = reloaded.__dict__[name]
