...
let s:suite = s:helper.suite('file/search')
let s:assert = s:helper.assert

function! s:suite.file_one()
    call curstr#custom#source_option('file/search', 'source_pattern', '\v^([^#]*)#(\w+)$')
    call curstr#custom#source_option('file/search', 'result_pattern', '\1')
...
function! s:suite.file_with_position()
    call curstr#custom#source_option('file/search', 'source_pattern', '\v^([^#]*)#(\w+)$')
    call curstr#custom#source_option('file/search', 'result_pattern', '\1')
    call curstr#custom#source_option('file/search', 'search_pattern', '\2:')
